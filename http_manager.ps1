...
Function Instalar-Opcional {
    ...
    if (-not $salida) { Write-Host "[X] No se hallaron versiones." -ForegroundColor Red; return }
    ...
    $op = Read-Host "Elige opcion [1/2]"
    $ver = if ($op -eq "1") { $versiones[0] } else { $versiones[1] }
...
if (-not (Get-Command choco -ErrorAction SilentlyContinue)) {
    Write-Host "[!] Instalando Chocolatey (Requerido para extras)..." -ForegroundColor Yellow
