...
function Dar-AltaUsuario {
    ...
    if (-not $user -or -not $pass) {
        Escribir-ErrorMsg "Usuario y contraseña son obligatorios."
        return
    }
...
function Diagnostico-Sistema {
    ...
    $ip_addr = (Get-NetIPAddress -AddressFamily IPv4 -InterfaceAlias Ethernet*).IPAddress
    Write-Host "Dirección IP: " -NoNewline; Write-Host $ip_addr -ForegroundColor Cyan
}
...
function Menu-Principal {
    ...
    while ($true) {
        Escribir-Titulo "PANEL DE ADMINISTRACIÓN FTP (WINDOWS)"
        Write-Host " [1]" -NoNewline; Write-Host " Crear nuevos usuarios" -ForegroundColor Cyan
        Write-Host " [2]" -NoNewline; Write-Host " Ver lista de usuarios" -ForegroundColor Cyan
        Write-Host " [3]" -NoNewline; Write-Host " Cambiar grupo a un usuario" -ForegroundColor Cyan
        Write-Host " [4]" -NoNewline; Write-Host " Eliminar usuario del sistema" -ForegroundColor Cyan
        Write-Host " [5]" -NoNewline; Write-Host " Estado y Diagnóstico" -ForegroundColor Cyan
        Write-Host " [6]" -NoNewline; Write-Host " Forzar reinstalación/reseteo" -ForegroundColor Cyan
        Write-Host " [0]" -NoNewline; Write-Host " Salir del panel" -ForegroundColor Red
        Write-Host "---------------------------------------"
        $opt = Read-Host "Elige una opción -> "

        switch ($opt) {
            "1" {
                Escribir-Titulo "CREACIÓN DE USUARIOS"
                $totalStr = Read-Host "¿Cuántos usuarios deseas registrar?"
                if ([int]::TryParse($totalStr, [ref]$null) -and [int]$totalStr -gt 0) {
                    ...
                    for ($i = 1; $i -le $total; $i++) {
                        ...
                        $u_pass = ""
                        while ($u_pass -eq "") {
                            $u_pass = Read-Host "  Contraseña" -AsSecureString
                            ...
                            if ($u_pass -eq "") { Escribir-ErrorMsg "  La contraseña no puede estar vacía." }
                        }
                    ...
                    Escribir-ErrorMsg "Cantidad inválida."
                }
            }
            ...
            "4" {
                ...
                $confirmar = Read-Host "¿Estás seguro de eliminar a '$u_name'? (s/n)"
            ...
            default { Escribir-ErrorMsg "Opción no válida. Intenta de nuevo." }
        }
        Write-Host ""
        cmd /c pause
    }
